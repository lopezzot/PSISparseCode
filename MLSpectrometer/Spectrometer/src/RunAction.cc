    ...
    : fDetector(detector)
{
    ...
    analysisManager->CreateNtuple("LayerSignals", "Gamma layer spectrometer data");
...
void RunAction::FillEvent(const G4Event* event, const EventAction& eventAction)
{
    auto* analysisManager = G4AnalysisManager::Instance();
    ...
    analysisManager->FillNtupleDColumn(3,
        eventAction.GetGeneratedMean() / MeV);
    analysisManager->FillNtupleDColumn(4,
        eventAction.GetGeneratedSigma() / MeV);
    ...
    for (G4int i = 0; i < SimulationConfig::NumberOfEnergyBins; ++i) {
        analysisManager->FillNtupleDColumn(
            firstSpectrumColumn + i, spectrum[i]);
    }
