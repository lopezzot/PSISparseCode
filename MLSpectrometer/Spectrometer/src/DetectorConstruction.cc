...
G4VPhysicalVolume *DetectorConstruction::Construct() {
  ...
  fWorldPhysicalVolume =
      new G4PVPlacement(nullptr, G4ThreeVector(), worldLogical, "WorldPhysical",
                        nullptr, false, 0, true);

  auto *layerSolid =
      new G4Box("LayerSolid", SimulationConfig::LayerSizeXY / 2.0,
                SimulationConfig::LayerSizeXY / 2.0,
                SimulationConfig::LayerThickness / 2.0);

  fLayerLogicalVolume =
      new G4LogicalVolume(layerSolid, plastic, "ScintillatorLayerLogical");
  ...
  for (G4int i = 0; i < SimulationConfig::NumberOfLayers; ++i) {
    const G4double z = firstLayerZ + i * (SimulationConfig::LayerThickness +
                                          SimulationConfig::LayerGap);

    new G4PVPlacement(nullptr, G4ThreeVector(0.0, 0.0, z), fLayerLogicalVolume,
                      "ScintillatorLayer", worldLogical, false, i, true);
  }
...
G4double DetectorConstruction::GetSourceZ() const { return -50.0 * mm; }
